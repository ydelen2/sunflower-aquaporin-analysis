...
#SBATCH --output=/work/dweikat/ydelen2/aquaporin_study/04_expression/logs/aqp_expr_v2_%j.out
#SBATCH --error=/work/dweikat/ydelen2/aquaporin_study/04_expression/logs/aqp_expr_v2_%j.err
...
if (file.exists(meta_file)) {
    ...
    # make sample names the row names so that metadata can be matched to count columns
    id_candidates <- intersect(c("sample", "sample_id", "run", "Run", "SRR", "sample_name"), colnames(sample_meta))
    if (length(id_candidates) > 0) {
        rownames(sample_meta) <- make.unique(as.character(sample_meta[[id_candidates[1]]]))
    } else if (!any(colnames(norm_counts) %in% rownames(sample_meta))) {
        first_match <- which(sapply(sample_meta, function(x) any(as.character(x) %in% colnames(norm_counts))))[1]
        if (!is.na(first_match)) rownames(sample_meta) <- make.unique(as.character(sample_meta[[first_match]]))
    }
    cat("Metadata samples matching expression columns:", sum(colnames(norm_counts) %in% rownames(sample_meta)), "/", ncol(norm_counts), "\n")
} else {
...
if ("stress" %in% colnames(sample_meta) || "condition" %in% colnames(sample_meta)) {
    ...
    col_meta[[cond_col_name]][is.na(col_meta[[cond_col_name]])] <- "unknown"
    stress_vals <- unique(col_meta[[cond_col_name]])
    n_stress <- length(stress_vals)
    # brewer palettes hold at most 12 colours; use hcl.colors beyond that
    stress_colors <- setNames(if (n_stress <= 12) brewer.pal(max(3, n_stress), "Set3")[1:n_stress] else hcl.colors(n_stress, "Dark 3"),
                              stress_vals)
    ...
    if ("tissue" %in% colnames(col_meta)) {
        col_meta$tissue[is.na(col_meta$tissue)] <- "unknown"
        tissue_vals <- unique(col_meta$tissue)
        n_tissue <- length(tissue_vals)
        tissue_colors <- setNames(if (n_tissue <= 8) brewer.pal(max(3, n_tissue), "Set2")[1:n_tissue] else hcl.colors(n_tissue, "Dark 2"),
                                  tissue_vals)
...
if (exists("cond_col_name")) {
    ...
    expr_long <- as.data.frame(heatmap_mat) %>%
        ...
        left_join(aqp_info %>% mutate(gene_id = rownames(aqp_info)) %>%
                  select(gene_id, !!sym(subfamily_col)),
                  by = "gene_id") %>%
        left_join(col_meta %>% mutate(sample = rownames(col_meta)) %>%
                  select(sample, !!sym(cond_col_name)),
                  by = "sample")

    if (length(name_col) > 0) {
        name_map <- setNames(paste(aqp_info[[name_col[1]]], rownames(aqp_info)), rownames(aqp_info))
    ...
    for (stress in stresses) {
        ...
        mean_sub$gene_label <- factor(mean_sub$gene_label,
                                       levels = unique(mean_sub$gene_label))
