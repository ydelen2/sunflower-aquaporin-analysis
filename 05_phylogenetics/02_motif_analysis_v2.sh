...
#SBATCH --output=/work/dweikat/ydelen2/aquaporin_study/logs/05_motif_v2_%j.out
#SBATCH --error=/work/dweikat/ydelen2/aquaporin_study/logs/05_motif_v2_%j.err
...
meme "${INPUT_FA}" \
    ...
    -nostatus \
    2>&1 | tee "${LOG_DIR}/meme_${SLURM_JOB_ID}.log" || true
# "|| true": the MEME module on this cluster fails in meme_xml_to_html (missing Perl
# XML::Parser::Expat) after meme.txt and meme.xml are complete; the HTML report is unused.

echo "[$(date '+%H:%M:%S')] MEME completed."

# Verify key output files exist
for f in "${MEME_OUT}/meme.xml" "${MEME_OUT}/meme.txt"; do
    if [[ ! -f "${f}" ]]; then
